      ...
      <section className="mb-8">
        <div className="flex items-center justify-start gap-32">
          <div className="flex flex-col items-start max-w-3xl ">
            <h1 className="text-5xl font-black text-green">
              Hey there, I'm Shubha!
            </h1>
            <p className="text-white/80 mt-5 text-lg">
              I'm a front-end developer focused on <i className='hover:text-green cursor-pointer'> performance, usability, and design</i>. From clean layouts to smooth interactions, I build interfaces that not only look good but work beautifully too.
            </p>
          </div>
        ...
        <div className="flex space-x-6   ">
          {categories.map((category, idx) => (
            <a
              key={idx}
              href={category.link}
              className="flex-shrink-0 w-64 rounded-xl bg-white/5 p-3 transition-transform duration-300 ease-in-out hover:shadow-spotify hover:ring-1 hover:ring-green    "
            >
              ...
              <h3 className="mt-4 text-white font-semibold text-lg text-center hover:text-green transition-colors">{category.title}</h3>
            </a>
          ))}
        </div>
      </section>
