...
const Projects = () => {
  ...
  return (
    <main className="flex-1 w-full max-w-full bg-gradient-to-b from-neutral-900 to-black text-white px-4 sm:px-6 pb-32 overflow-x-hidden mt-12">
      {/* Header */}
      ...
      <div className=" scrollbar-hide -mx-2 sm:mx-0">
        <div className="flex sm:grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 px-2 sm:px-0">
          {projects.map((project, index) => (
            <div
              key={index}
              className="bg-neutral-800 hover:ring-1 hover:ring-green transform transition duration-300 ease-in-out rounded-lg p-4 min-w-[280px] sm:min-w-0 flex-shrink-0"
            >
              <img
                src={project.image}
                alt={project.title}
      ...
      <section className="mt-12 p-4 sm:p-6 bg-neutral-900 rounded-lg w-full overflow-x-auto scrollbar-hide">
        <h2 className="text-2xl font-bold mb-4">Personal Projects Playlist</h2>
        <table className="min-w-[700px] w-full table-auto border-collapse text-left text-white">
