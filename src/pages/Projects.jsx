...
<section className="px-0 sm:px-8 py-6">
  <h2 className="text-xl font-bold mb-4 px-4 sm:px-0">Projects I've Built</h2>
    ...
    >
      {projects.map((project, index) => (
        <div
          key={index}
          tabIndex={0}
          className="bg-neutral-800 rounded-lg p-4 min-w-full max-w-full sm:min-w-0 sm:max-w-none flex-shrink-0 shadow-lg snap-start hover:ring-1 hover:ring-green transition duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-green-500"
        >
          <img
            src={project.image}
            alt={project.title}
          ...
          />
          <h2 className="text-base font-semibold mb-1 hover:text-green">{project.title}</h2>
...
<section className="mt-12 w-full overflow-x-auto rounded-lg bg-neutral-900 sm:p-6 p-4 scrollbar-spotify scrollbar-no-arrows">
  ...
  <div className="min-w-[700px] w-full inline-block align-middle">
    <table className="w-full table-auto border-collapse text-left text-white">
      <thead className="border-b border-gray-700 sticky top-0 bg-neutral-900 z-10">
        <tr>
      ...
      <tbody>
        {personalProjects.map((project, i) => (
          <tr
            key={i}
            className="group border-b border-gray-800 cursor-pointer transition hover:bg-neutral-800"
